...
void test_evaluator() {
    ...
    assert(eval == Evaluator::CHECKMATE);
...
void test_search_starting_position() {
    ...
    Move best = engine.find_best_move(board, 2);
...
void test_search_depth() {
    ...
    for (int depth = 1; depth <= 4; ++depth) {
        board.reset();
        Move best = engine.find_best_move(board, depth);
        
        std::cout << "Depth " << depth << ": " << engine.nodes_evaluated() 
